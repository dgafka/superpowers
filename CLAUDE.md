...
**Workflow:** `brainstorming` develops an approved design in conversation. The manual-only `orchestration-research` skill then coordinates named Orca research tasks, user-approved implementation tasks, separate Codex sub-worktrees, worker questions, and native GitHub stacked PRs. Each implementation worker invokes `executing-tasks` and runs directly in its assigned sub-worktree. No generated design or task documents mediate the workflow.

**Research orchestration:** the main worktree remains the research and coordination surface. Independent research and implementation tasks may run concurrently. Every implementation task receives a short `<action>-<topic>` name reused for its Orca task and sub-worktree, and requires user confirmation before dispatch. Dependent tasks use prerequisite branches as their Git bases.
...
- `brainstorming` ends with the approved conversational design and directs the user to invoke `orchestration-research`; it does not dispatch implementation.
- The main research session never implements code. Each approved implementation runs through `executing-tasks` in its own Orca sub-worktree.
