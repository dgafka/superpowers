...
**Workflow:** `brainstorming` develops an approved design in conversation. The reusable `orchestrator-agent` skill coordinates user-approved implementation tasks, separate Codex sub-worktrees, explicitly requested review sessions, worker questions, and native GitHub stacked PRs. It is directly user-invokable and also callable by `orchestration-research`, which coordinates read-only research before handing over its synthesized task DAG. Each implementation worker invokes `orchestrator-subworktree-task` directly in its assigned sub-worktree. No generated design or task documents mediate the workflow.

**Orchestration:** the main worktree remains the coordination surface. Independent implementation tasks may run concurrently. Every implementation task receives an `implement-<topic>` name reused for its Orca task and sub-worktree, requires user confirmation before dispatch, and explicitly instructs its worker to invoke the isolated `test-driven-development` skill and follow RED-GREEN-REFACTOR. On explicit request, a dependent `review-<topic>` task runs in a separate Codex session against that implementation's existing sub-worktree; approved fixes return to the original worker. Dependent implementation tasks use prerequisite branches as their Git bases. `orchestration-research` adds concurrent read-only research before reusing this implementation workflow.

**Review skill mapping:** `review-changes` supports direct use and explicitly requested Orca `review-<topic>` sessions. It scans available skills non-interactively during its understanding phase, invokes only relevant research or domain guidance, and uses it to enrich the review. It does not generate proposals or ask the user to approve a skill list.
...
**No `commands/` directory — everything is a skill.** User-triggered workflows live in `skills/`. `create-pull-request`, `improve-workflow`, `cleanup-worktree`, and `orchestration-research` are manual-only on both platforms: `disable-model-invocation: true` for Claude Code plus `agents/openai.yaml` with `policy.allow_implicit_invocation: false` for Codex. `orchestrator-agent` and `review-changes` remain agent-invocable for skill reuse and are also directly user-invokable. There are deliberately no `~/.codex/prompts` wrappers.
