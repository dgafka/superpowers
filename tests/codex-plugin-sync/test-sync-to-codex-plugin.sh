...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/dgafka/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/dgafka/.fixture-keep"
    cat > "$repo/plugins/dgafka/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/dgafka/.fixture-keep
    git -C "$repo" add plugins/dgafka/skills/example/SKILL.md
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/dgafka/.codex-plugin" \
        "$repo/plugins/dgafka/.private-journal" \
        "$repo/plugins/dgafka/assets" \
        "$repo/plugins/dgafka/skills/example"

    cat > "$repo/plugins/dgafka/.codex-plugin/plugin.json" <<EOF
{
  "name": "dgafka",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/dgafka/assets/superpowers-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/dgafka/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/dgafka/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/dgafka/.codex-plugin/plugin.json \
        plugins/dgafka/assets/app-icon.png \
        plugins/dgafka/assets/superpowers-small.svg \
        plugins/dgafka/skills/example/SKILL.md \
        plugins/dgafka/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/dgafka/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/dgafka/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/dgafka/.private-journal/leak.txt"
    git -C "$repo" add plugins/dgafka/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/dgafka/skills/example/SKILL.md"
    ...
    assert_branch_absent "$dest" "sync/dgafka-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/dgafka/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/dgafka" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/dgafka-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/dgafka'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/dgafka-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/dgafka-*" "Clean no-op local apply does not create sync branch in destination checkout"
