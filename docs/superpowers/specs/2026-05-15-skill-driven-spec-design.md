...
- **Match rule:** conservative. Prefer fewer, high-signal matches over a long list. `superpowers:test-driven-development` and `superpowers:verification-before-completion` apply to nearly every spec and should default in as **behaviour-driving** unless clearly inapplicable.
