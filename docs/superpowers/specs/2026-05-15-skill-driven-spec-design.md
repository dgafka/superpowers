...
- The subagent **categorizes** each matched skill into exactly one of three groups: **spec-driving** (contributes to spec content only — no runtime use), **behaviour-driving** (drives runtime discipline only — minimal/no spec body contribution), or **spec-behaviour-driving** (both — bakes content into the spec AND drives runtime behaviour). Only behaviour-driving and spec-behaviour-driving skills appear in the spec's `## Required Skills` block; spec-driving skills are excluded because the executor has no runtime job for them — their value was already extracted into the spec body during brainstorming.
...
The Required Skills block format inside specs is unchanged from today (skill name + one-sentence rationale, no category tag). Only the timing, source, and category-based filter applied to which skills are listed change.
...
- **Per matched skill, the subagent returns four fields:**
  - `name` — skill identifier (e.g., `superpowers:test-driven-development`).
  - `category` — exactly one of `spec-driving`, `behaviour-driving`, `spec-behaviour-driving` (definitions below).
...
- **Category definitions** (must appear in the subsection body and in the prompt):

  | Category | When to use | Example skill / scenario | Listed in `## Required Skills`? |
  |---|---|---|---|
  | **spec-driving** | The skill's value is to *gather or shape information* that the brainstormer bakes into the spec. The executor has no runtime job for the skill because the gathered output is already in the spec body. | "Collect comments on related pull requests and surface anything worth investigating" — the output becomes spec context, no runtime invocation needed. | No |
  | **behaviour-driving** | The skill governs *how* the executor works at runtime. Minimal or no contribution to spec body — listing the skill is the requirement. | `superpowers:test-driven-development` — RED → GREEN → REFACTOR at runtime; the spec doesn't need to restate the cycle. | Yes |
  | **spec-behaviour-driving** | The skill bakes content into the spec (examples, structural requirements, acceptance items) *and* governs runtime behaviour the executor must apply throughout implementation. | "When writing tests, always test on the edges" — spec lists the edge cases; the executor follows the discipline at runtime. | Yes |

- **Match rule:** conservative. Prefer fewer, high-signal matches over a long list. `superpowers:test-driven-development` applies to nearly every spec and should default in as **behaviour-driving** unless clearly inapplicable.
...
- **Category does not affect re-investigation.** Every expectation, regardless of source-skill category, is walked through the four-state table in Change 5. The category is consulted only at Step 8 (Documentation), to filter which skills appear in the `## Required Skills` block.

Subagent prompt template to embed verbatim in the SKILL.md:

> Read the following approved design summary end-to-end. Scan the provided skill list and identify which skills apply to *this specific* implementation. Be conservative — prefer fewer, high-signal matches. `superpowers:test-driven-development` applies to nearly every spec and should default in (category: `behaviour-driving`) unless clearly inapplicable.
>
...
>
> Read the skill file end-to-end and extract concrete expectations the spec should address. Each expectation should be a single bullet stating something the spec ought to specify (e.g., "Acceptance Criteria must enumerate evidence", "Testing must define per-task verification approach").
>
> Return a structured response. For each matched skill, include: `name` (skill identifier), `category` (one of the three above), `why_relevant` (one sentence specific to this design), `expectations` (bullet list). Do not edit any file.
>
...
Update the writing step so the Required Skills block is written inline as part of the initial spec write, using the Step 6 mapping output **filtered by category**.

**Filter rule:** the block lists *only* skills whose `category` is `behaviour-driving` or `spec-behaviour-driving`. Skills categorized `spec-driving` are excluded — their contribution is already in the spec body and they have no runtime job. Do not list spec-driving skills in any auxiliary section either; the spec is silent about them.

The block format remains identical to today (no category tag in the rendered output):
...
**Zero-listable-skills fallback:** if after filtering the block would contain no entries (every matched skill was `spec-driving`, or the mapping subagent returned an empty list), the block becomes:
...
The spec body itself (Acceptance Criteria, Testing, Scope, etc.) already reflects the skill-driven refinements applied during re-investigation — including refinements derived from `spec-driving` skills. No separate post-write skill mapping pass.
...
> **Spec skill mapping.** During brainstorming, after the user verbally approves the design but before the spec file is written, a `general-purpose` subagent scans the available skills list against the approved design summary and returns structured expectations per matched skill, with each skill assigned exactly one category: `spec-driving` (shapes spec content only), `behaviour-driving` (drives runtime discipline only), or `spec-behaviour-driving` (both). The subagent edits no files. The brainstormer then runs a re-investigation pass: technical-detail gaps (verification evidence, testing details, scope refinements) are applied silently to the spec being drafted; gaps that would change behaviour, flow, or business logic trigger one-at-a-time clarifying questions to the user. The `## Required Skills` block is written inline as part of the initial spec write and lists only the `behaviour-driving` and `spec-behaviour-driving` skills (the executor has no runtime job for `spec-driving` skills — their value is already baked into the spec body). There is no second mapping pass after the spec exists.
...
4. The Skill Mapping subsection in `skills/brainstorming/SKILL.md` specifies that the subagent prompt MUST include (a) a structured design summary instead of a spec path, (b) the parent's verbatim skill list from its `<system-reminder>`, and (c) the instruction set. The subagent returns `{ name, category, why_relevant, expectations }` per matched skill — `category` is exactly one of `spec-driving`, `behaviour-driving`, `spec-behaviour-driving` — and edits no files. The subsection defines all three categories with one-line descriptions and example scenarios. The default-in line for `superpowers:test-driven-development` is preserved and notes that it defaults to category `behaviour-driving`.
...
6. The Documentation subsection in `skills/brainstorming/SKILL.md` instructs that the `## Required Skills` block is written inline as part of the initial spec write using the Step 6 mapping output **filtered to skills whose category is `behaviour-driving` or `spec-behaviour-driving`** (skills categorized `spec-driving` are excluded from the block). The subsection shows the unchanged block format and the zero-listable-skills fallback that triggers when filtering leaves no entries.
7. `CLAUDE.md`'s "Spec skill mapping" paragraph reflects the pre-write timing, the re-investigation step, the technical-vs-behaviour/flow/business-logic split, and the three-category classification with its filter rule for the `## Required Skills` block.
...
- Re-read `skills/brainstorming/SKILL.md` end-to-end after edits. Confirm: checklist order matches Acceptance Criterion 1; graphviz block matches Acceptance Criterion 2; "After the Design" subsection order matches Acceptance Criterion 3; the mapping subagent prompt takes a design summary and returns a structured response including a `category` field; the three-category definition table (spec-driving / behaviour-driving / spec-behaviour-driving) is present in the Skill Mapping subsection; the Documentation subsection's filter rule (only `behaviour-driving` and `spec-behaviour-driving` skills listed in `## Required Skills`) is present; the re-investigation four-state table is present with the one-sentence heuristic.
