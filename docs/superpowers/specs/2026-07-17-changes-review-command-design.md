...
- **Summarize** the change for the user:
  - **Problem space** — what existed before and why it needed to change. Include a Mermaid diagram of the *prior* flow whenever a flow/state/pipeline is involved.
  - **Solution space** — the high-level approach taken. Include a Mermaid diagram of the *resulting* flow under the same condition. (Deliberately both old-flow-and-new-flow together here — unlike `create-pull-request.md`'s after-only convention, this phase's goal is comprehension, not PR-body brevity. This old+new pairing applies everywhere a flow diagram appears in this command's output, including inside Phase 2 findings that reference a flow change.)
