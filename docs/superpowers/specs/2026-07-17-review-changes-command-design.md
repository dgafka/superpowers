...
- **Summarize** the change for the user:
  ...
  - **Ground both spaces in the actual diff**, not just prose: pull short code snippets from the changed lines to substantiate each claim in the problem-space/solution-space narrative, so the high-level explanation and the underlying code stay connected and the user can jump from one to the other. When the logic a snippet would show is long or intricate, replace the raw snippet with a short pseudocode sketch of just the relevant control flow instead of pasting the whole thing.
